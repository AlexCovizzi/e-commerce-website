...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`BookView` (`isbn` INT, `title` INT, `description` INT, `pages` INT, `price` INT, `publication_date` INT, `language` INT, `publisher_name` INT, `stock` INT, `vote` INT, `n_votes` INT, `coverUri` INT, `timestamp` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`OrderView`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`OrderView` (`id` INT, `user_id` INT, `user_name` INT, `user_surname` INT, `created` INT, `tot_price` INT, `shipping_cost` INT, `state` INT, `coupon_code` INT, `receiver_name` INT, `add1` INT, `add2` INT, `city` INT, `province` INT, `country` INT, `cap` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`VoteView`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`VoteView` (`book_isbn` INT, `title` INT, `user_id` INT, `name` INT, `surname` INT, `thumb_up` INT, `comment` INT);
...
-- View `mydb`.`VoteView`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`VoteView`;
USE `mydb`;
CREATE  OR REPLACE VIEW `VoteView` AS
SELECT V.book_isbn, B.title, V.user_id, U.name, U.surname, V.thumb_up, V.comment
FROM Vote as V
